...
function CrudPage({ type, title, icon, blank }) {
  ...
  const save = async (form) => {
    editing
      ? await api.put(`/${type}/${editing._id}`, form)
    ...
    <div className="page">
      <section className="subhero">
        <div className="eyebrow">
          <span>{type === "patients" ? "03" : "05"}</span> Database records
        </div>
        <h1>
          {title},<br />
          <em>organized.</em>
      ...
      <div className="data-table">
        {items.length ? (
          items.map((item) => (
            <div className="data-row" key={item._id}>
              ...
              <div>
                ...
                <span>{type === "patients" ? item.phone : item.diagnosis}</span>
              </div>
              <div>
                <small>{type === "patients" ? "Email" : "Doctor"}</small>
                <span>
                  {type === "patients"
                    ? show(item.email)
                    : show(item.doctor?.name)}
                </span>
              </div>
              <div>
                <small>
                  {type === "patients" ? "Birth date" : "Visit date"}
                </small>
                <span>
                  {showDate(
                    type === "patients" ? item.birthDate : item.visitDate,
                  )}
                </span>
              </div>
          ...
          <div className="empty">No database records found.</div>
        )}
  ...
  return createPortal(
    <div className="modal-backdrop details-backdrop">
      <article
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`${type === "patients" ? "Patient" : "Medical record"} details`}
      >
        ...
        <section>
          ...
          <dl className="details-grid">
            ...
            <div>
              <dt>Email</dt>
              <dd>{show(patient?.email)}</dd>
            </div>
            <div>
              <dt>Phone</dt>
              <dd>{show(patient?.phone)}</dd>
            </div>
            <div>
              <dt>Birth date</dt>
              <dd>{showDate(patient?.birthDate)}</dd>
            </div>
            <div>
              <dt>Sex</dt>
              <dd>{show(patient?.sex)}</dd>
            </div>
            <div className="details-wide">
              <dt>Address</dt>
              <dd>{show(patient?.address)}</dd>
            </div>
            {type === "patients" && (
              <div className="details-wide">
        ...
        {type === "records" && (
          <section className="clinical-details">
            ...
            <dl className="details-grid">
              <div>
                <dt>Visit date</dt>
                <dd>{showDate(item.visitDate)}</dd>
              </div>
              <div>
                <dt>Assigned doctor</dt>
                <dd>{show(item.doctor?.name)}</dd>
              </div>
              <div className="details-wide">
                <dt>Specialty</dt>
                <dd>{show(item.doctor?.specialty)}</dd>
              </div>
              <div className="details-wide">
                <dt>Diagnosis</dt>
...
function DataModal({ type, item, blank, onClose, onSave }) {
  const normalize = () => {
    ...
    if (type === "patients")
      return {
        ...
        email: item.email || "",
        phone: item.phone,
        birthDate: new Date(item.birthDate).toISOString().slice(0, 10),
        sex: item.sex,
    ...
    return {
      ...
      visitDate: new Date(item.visitDate).toISOString().slice(0, 10),
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError(e.response?.data?.errors?.[0]?.msg || "Unable to save record.");
    }
  };
  return (
    <div className="modal-backdrop">
      <form className="modal" onSubmit={submit}>
        ...
        <div className="eyebrow">
          {item ? "Update" : "Create"} database record
        </div>
        <h2>
          {item ? "Edit" : "Add"}{" "}
          {type === "patients" ? "patient" : "medical record"}
        </h2>
        {type === "patients" ? (
          <>
            ...
            <div className="form-row">
              {field("birthDate", "Birth date", { type: "date" })}
              <label>
                Sex
                <select
                  value={form.sex}
          ...
          <>
            ...
            {field("visitDate", "Visit date", { type: "date" })}
            {field("diagnosis", "Diagnosis")}
            {field("treatment", "Treatment")}
            {field("notes", "Notes", { required: false })}
          </>
        )}
